...
pressure_levels = np.array([1000, 950, 900, 800, 700, 600, 500,
                           400, 350, 300, 250, 200, 150, 100, 75, 50, 25, 10, 5, 2, 1])
pressure_levels *= 100
nlevels = len(pressure_levels)

# timestep for initial period where the model only calculates radiative effects
dt_spinup = 60*17.2
# timestep for the main sequence where the model calculates velocities/advection
dt_main = 60*7.2
spinup_length = 0*day 				# how long the model should only calculate radiative effects

###

# you probably won't need this, but there is the option to smooth out fields using FFTs (NB this slows down computation considerably and can introduce nonphysical errors)
smoothing = False
...
# how far north polar plane data is calculated from the south pole (do not set this beyond 45!) [mirrored to north pole as well]
pole_lower_latitude_limit = -75
# how far south regular gridded data is calculated (do not set beyond about 80) [also mirrored to north pole]
pole_higher_latitude_limit = -85
# at what pressure should the equations of motion stop being applied (to absorb upwelling atmospheric waves) [hPa]
sponge_layer = 10
...
if not load:
	...
	temp_profile = np.interp(
		x=pressure_levels[::-1], xp=standard_pressure[::-1], fp=standard_temp[::-1])[::-1]
...
if initial_setup:
	...
	albedo = np.random.uniform(-albedo_variance,
	                           albedo_variance, (nlat, nlon)) + 0.2
...
if setup_grids:
	...
	size_of_grid = planet_radius * \
		np.cos(lat[pole_low_index_S+grid_pad]*np.pi/180.0)

	### south pole ###
	grid_x_values_S = np.arange(-size_of_grid,
	                            size_of_grid, polar_grid_resolution)
	grid_y_values_S = np.arange(-size_of_grid,
	                            size_of_grid, polar_grid_resolution)
	grid_xx_S, grid_yy_S = np.meshgrid(grid_x_values_S, grid_y_values_S)

	grid_side_length = len(grid_x_values_S)

	grid_lat_coords_S = (-np.arccos(((grid_xx_S**2 + grid_yy_S**2)
	                     ** 0.5)/planet_radius)*180.0/np.pi).flatten()
	grid_lon_coords_S = (
		180.0 - np.arctan2(grid_yy_S, grid_xx_S)*180.0/np.pi).flatten()
	...
	for i in range(pole_low_index_S):
		for j in range(nlon):
			polar_x_coords_S.append(
				planet_radius*np.cos(lat[i]*np.pi/180.0)*np.sin(lon[j]*np.pi/180.0))
			polar_y_coords_S.append(-planet_radius *
			                        np.cos(lat[i]*np.pi/180.0)*np.cos(lon[j]*np.pi/180.0))
	...
	grid_x_values_N = np.arange(-size_of_grid,
	                            size_of_grid, polar_grid_resolution)
	grid_y_values_N = np.arange(-size_of_grid,
	                            size_of_grid, polar_grid_resolution)
	grid_xx_N, grid_yy_N = np.meshgrid(grid_x_values_N, grid_y_values_N)

	grid_lat_coords_N = (np.arccos((grid_xx_N**2 + grid_yy_N**2)
	                     ** 0.5/planet_radius)*180.0/np.pi).flatten()
	grid_lon_coords_N = (
		180.0 - np.arctan2(grid_yy_N, grid_xx_N)*180.0/np.pi).flatten()
	...
	for i in np.arange(pole_low_index_N, nlat):
		for j in range(nlon):
			polar_x_coords_N.append(
				planet_radius*np.cos(lat[i]*np.pi/180.0)*np.sin(lon[j]*np.pi/180.0))
			polar_y_coords_N.append(-planet_radius *
			                        np.cos(lat[i]*np.pi/180.0)*np.cos(lon[j]*np.pi/180.0))

	indices = (pole_low_index_N, pole_high_index_N,
	           pole_low_index_S, pole_high_index_S)
	...
	coriolis_plane_N = low_level.beam_me_up_2D(
		lat[(pole_low_index_N-grid_pad):], lon, data, grids[0], grid_lat_coords_N, grid_lon_coords_N)
	...
	coriolis_plane_S = low_level.beam_me_up_2D(
		lat[:(pole_low_index_S+grid_pad)], lon, data, grids[1], grid_lat_coords_S, grid_lon_coords_S)
...
if load:
	...
	potential_temperature, temperature_world, u, v, w, x_dot_N, y_dot_N, x_dot_S, y_dot_S, t, albedo, tracer = pickle.load(
		open("save_file.p", "rb"))
...
if plot:
	if not diagnostic:
		...
		ax[0].streamplot(lon_plot, lat_plot, u[:, :, 0],
		                 v[:, :, 0], color='white', density=1)
		test = ax[1].contourf(heights_plot, lat_z_plot, np.transpose(np.mean(low_level.theta_to_t(
			potential_temperature, pressure_levels), axis=1))[:top, :], cmap='seismic', levels=15)
		ax[1].contour(heights_plot, lat_z_plot, np.transpose(np.mean(u, axis=1))[
		              :top, :], colors='white', levels=20, linewidths=1, alpha=0.8)
		ax[1].quiver(heights_plot, lat_z_plot, np.transpose(np.mean(v, axis=1))[
		             :top, :], np.transpose(np.mean(100*w, axis=1))[:top, :], color='black')
		...
		f.canvas.set_window_title('CLAuDE')
		ax[0, 0].contourf(heights_plot, lat_z_plot, np.transpose(
			np.mean(u, axis=1))[:top, :], cmap='seismic')
		ax[0, 0].set_title('u')
		ax[0, 1].contourf(heights_plot, lat_z_plot, np.transpose(
			np.mean(v, axis=1))[:top, :], cmap='seismic')
		ax[0, 1].set_title('v')
		ax[1, 0].contourf(heights_plot, lat_z_plot, np.transpose(
			np.mean(w, axis=1))[:top, :], cmap='seismic')
		ax[1, 0].set_title('w')
		ax[1, 1].contourf(heights_plot, lat_z_plot, np.transpose(
			np.mean(atmosp_addition, axis=1))[:top, :], cmap='seismic')
	...
	if level_plots:
		...
		for k, z in zip(range(nplots), level_plots_levels):
			z += 1
			bx[k].contourf(lon_plot, lat_plot,
			               potential_temperature[:, :, z], cmap='seismic')
...
def plotting_routine():
	...
	if plot:
		...
		if not diagnostic:
			...
			ax[0].contour(lon_plot, lat_plot, tracer[:, :, sample_level],
			              alpha=0.5, antialiased=True, levels=np.arange(0.01, 1.01, 0.01))
			if velocity:
				ax[0].quiver(lon_plot[::quiver_padding, ::quiver_padding], lat_plot[::quiver_padding, ::quiver_padding],
				             u[::quiver_padding, ::quiver_padding, sample_level], v[::quiver_padding, ::quiver_padding, sample_level], color='white')
			ax[0].set_xlim((lon.min(), lon.max()))
			...
			ax[1].contour(heights_plot, lat_z_plot, np.transpose(np.mean(tracer, axis=1))[
			              :top, :], alpha=0.5, antialiased=True, levels=np.arange(0.001, 1.01, 0.01))

			if velocity:
				ax[1].contour(heights_plot, lat_z_plot, np.transpose(np.mean(u, axis=1))[
				              :top, :], colors='white', levels=20, linewidths=1, alpha=0.8)
				ax[1].quiver(heights_plot, lat_z_plot, np.transpose(np.mean(v, axis=1))[
				             :top, :], np.transpose(np.mean(-10*w, axis=1))[:top, :], color='black')
		...
		if level_plots:
			for k, z in zip(range(nplots), level_plots_levels):
				z += 1
				bx[k].contourf(lon_plot, lat_plot, potential_temperature[:,
				               :, z], cmap='seismic', levels=15)
				bx[k].quiver(lon_plot[::quiver_padding, ::quiver_padding], lat_plot[::quiver_padding, ::quiver_padding],
				             u[::quiver_padding, ::quiver_padding, z], v[::quiver_padding, ::quiver_padding, z], color='white')
	...
	if above and velocity:
		...
		if pole == 's':
			...
			gx[0].contourf(lon, lat[:pole_low_index_S],
			               potential_temperature[:pole_low_index_S, :, above_level])

			gx[1].set_title('polar_plane_advect')
			polar_temps = low_level.beam_me_up(
				lat[:pole_low_index_S], lon, potential_temperature[:pole_low_index_S, :, :], grids[1], grid_lat_coords_S, grid_lon_coords_S)
			output = low_level.beam_me_up_2D(
				lat[:pole_low_index_S], lon, w[:pole_low_index_S, :, above_level], grids[1], grid_lat_coords_S, grid_lon_coords_S)

			gx[1].contourf(grid_x_values_S/1E3, grid_y_values_S/1E3, output)
			gx[1].contour(grid_x_values_S/1E3, grid_y_values_S/1E3, polar_temps[:,
			              :, above_level], colors='white', levels=20, linewidths=1, alpha=0.8)
			gx[1].quiver(grid_x_values_S/1E3, grid_y_values_S/1E3,
			             x_dot_S[:, :, above_level], y_dot_S[:, :, above_level])

			gx[1].add_patch(plt.Circle((0, 0), planet_radius *
			                np.cos(lat[pole_low_index_S]*np.pi/180.0)/1E3, color='r', fill=False))
			gx[1].add_patch(plt.Circle((0, 0), planet_radius *
			                np.cos(lat[pole_high_index_S]*np.pi/180.0)/1E3, color='r', fill=False))
			...
			gx[2].contourf(lon, lat[:pole_low_index_S],
			               u[:pole_low_index_S, :, above_level])
			gx[2].quiver(lon[::5], lat[:pole_low_index_S], u[:pole_low_index_S,
			             ::5, above_level], v[:pole_low_index_S, ::5, above_level])
		else:
			gx[0].set_title('temperature')
			gx[0].contourf(lon, lat[pole_low_index_N:],
			               potential_temperature[pole_low_index_N:, :, above_level])

			gx[1].set_title('polar_plane_advect')
			polar_temps = low_level.beam_me_up(lat[pole_low_index_N:], lon, np.flip(
				potential_temperature[pole_low_index_N:, :, :], axis=1), grids[0], grid_lat_coords_N, grid_lon_coords_N)
			output = low_level.beam_me_up_2D(lat[pole_low_index_N:], lon, atmosp_addition[pole_low_index_N:,
			                                 :, above_level], grids[0], grid_lat_coords_N, grid_lon_coords_N)
			output = low_level.beam_me_up_2D(
				lat[pole_low_index_N:], lon, w[pole_low_index_N:, :, above_level], grids[0], grid_lat_coords_N, grid_lon_coords_N)

			gx[1].contourf(grid_x_values_N/1E3, grid_y_values_N/1E3, output)
			gx[1].contour(grid_x_values_N/1E3, grid_y_values_N/1E3, polar_temps[:,
			              :, above_level], colors='white', levels=20, linewidths=1, alpha=0.8)
			gx[1].quiver(grid_x_values_N/1E3, grid_y_values_N/1E3,
			             x_dot_N[:, :, above_level], y_dot_N[:, :, above_level])

			gx[1].add_patch(plt.Circle((0, 0), planet_radius *
			                np.cos(lat[pole_low_index_N]*np.pi/180.0)/1E3, color='r', fill=False))
			gx[1].add_patch(plt.Circle((0, 0), planet_radius *
			                np.cos(lat[pole_high_index_N]*np.pi/180.0)/1E3, color='r', fill=False))

			gx[2].set_title('south_addition_smoothed')
			# gx[2].contourf(lon,lat[pole_low_index_N:],north_addition_smoothed[:,:,above_level])
			gx[2].contourf(lon, lat[pole_low_index_N:],
			               u[pole_low_index_N:, :, above_level])
			gx[2].quiver(lon[::5], lat[pole_low_index_N:], u[pole_low_index_N:,
			             ::5, above_level], v[pole_low_index_N:, ::5, above_level])
...
def timestep():
	...
	print('U: ', round(u[:, :, :sponge_index-1].max(), 2), ' - ', round(u[:, :, :sponge_index-1].min(), 2), ' V: ', round(v[:, :, :sponge_index-1].max(), 2),
	      ' - ', round(v[:, :, :sponge_index-1].min(), 2), ' W: ', round(w[:, :, :sponge_index-1].max(), 2), ' - ', round(w[:, :, :sponge_index-1].min(), 4))
	...
	temperature_world, potential_temperature = top_level.radiation_calculation(
		temperature_world, potential_temperature, pressure_levels, heat_capacity_earth, albedo, insolation, lat, lon, t, dt, day, year, axial_tilt)
	if smoothing:
		potential_temperature = top_level.smoothing_3D(
			potential_temperature, smoothing_parameter_t)
	if verbose:
	...
	for k in np.arange(1, nlevels):
		geopotential[:, :, k] = geopotential[:, :, k-1] - \
			potential_temperature[:, :, k]*(sigma[k]-sigma[k-1])
	...
	if velocity:
		...
		u_add, v_add = top_level.velocity_calculation(
			u, v, w, pressure_levels, geopotential, potential_temperature, coriolis, gravity, dx, dy, dt, sponge_index)
		...
		u_add, v_add, x_dot_N, y_dot_N, x_dot_S, y_dot_S = top_level.polar_planes(u, v, u_add, v_add, potential_temperature, geopotential, grid_velocities, indices,
		                                                                          grids, coords, coriolis_plane_N, coriolis_plane_S, grid_side_length, pressure_levels, lat, lon, dt, polar_grid_resolution, gravity, sponge_index)
		...
		x_dot_N, y_dot_N, x_dot_S, y_dot_S = top_level.update_plane_velocities(lat, lon, pole_low_index_N, pole_low_index_S, np.flip(u[pole_low_index_N:, :, :], axis=1), np.flip(
			v[pole_low_index_N:, :, :], axis=1), grids, grid_lat_coords_N, grid_lon_coords_N, u[:pole_low_index_S, :, :], v[:pole_low_index_S, :, :], grid_lat_coords_S, grid_lon_coords_S)
		grid_velocities = (x_dot_N, y_dot_N, x_dot_S, y_dot_S)
		...
		w = -top_level.w_calculation(u, v, w, pressure_levels, geopotential, potential_temperature, coriolis,
		                             gravity, dx, dy, dt, indices, coords, grids, grid_velocities, polar_grid_resolution, lat, lon)
		if smoothing:
		...
		atmosp_addition = top_level.divergence_with_scalar(
			potential_temperature, u, v, w, dx, dy, lat, lon, pressure_levels, polar_grid_resolution, indices, coords, grids, grid_velocities)

		if smoothing:
			atmosp_addition = top_level.smoothing_3D(
				atmosp_addition, smoothing_parameter_add)
		...
		tracer_addition = top_level.divergence_with_scalar(
			tracer, u, v, w, dx, dy, lat, lon, pressure_levels, polar_grid_resolution, indices, coords, grids, grid_velocities)
	...
	if save:
		if t-last_save >= save_frequency*dt:
			pickle.dump((potential_temperature, temperature_world, u, v, w, x_dot_N,
			            y_dot_N, x_dot_S, y_dot_S, t, albedo, tracer), open("save_file.p", "wb"))
			last_save = t
